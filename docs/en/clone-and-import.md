...
`git clone https://github.com/ByteLegendQuest/java-fix-uninitialized-field-npe`

A project directory named `java-fix-uninitialized-field-npe` will be created, which can be moved wherever you like.

Open `IDEA`, `File` - `Open`, select and open the `pom.xml` in `java-fix-uninitialized-field-npe` directory which is created just now.
